...
import { buildTodayFallbackPayload } from "../_fallback";
import {
...
export async function GET(request: Request) {
  ...
  for (const backendBaseUrl of getCandidateBackendBaseUrls()) {
    try {
      const candidate = await fetchBackendInsights({
        backendBaseUrl,
        endpointPath: "/ai-insights/today",
        suffix,
  ...
  if (!upstreamResponse) {
    const fallbackReason = sawBackend404
      ? "AI insights backend endpoint not available; using resilient Supabase fallback."
      : "AI insights backend unreachable; using resilient Supabase fallback.";
    const {
      data: { user },
    ...
    try {
      const fallbackPayload = await buildTodayFallbackPayload({
        supabase,
        userId: user.id,
        date,
        timeZone: timezone,
      ...
      const message = error instanceof Error ? error.message : "Unable to build fallback AI insights.";
      return NextResponse.json(
        {
          detail: `Unable to reach AI insights backend and fallback failed: ${message}`,
        },
        { status: 502 }
      );
    }
