...
export async function POST(request: Request, { params }: { params: { provider: string } }) {
  ...
  const { upstream, sawBackend404 } = await fetchWithBackendFailover({
    accessToken,
    endpointPath: `/integrations/${provider}/connect`,
    method: "POST",
    body,
  });
  if (!upstream) {
    return NextResponse.json(
      {
        detail: sawBackend404
          ? "Integration connect endpoint is updating. Please try again shortly."
          : "Unable to start integration right now. Please try again shortly.",
      },
