...
export async function GET(request: Request) {
  ...
  const { upstream, sawBackend404 } = await fetchWithBackendFailover({
    accessToken,
    endpointPath: "/health-data/activity",
    method: "GET",
    suffix,
  });
  if (!upstream) {
    return NextResponse.json(
      {
        detail: sawBackend404
          ? "Health activity endpoint is updating. Please try again shortly."
          : "Unable to load health activity right now. Please try again shortly.",
      },
