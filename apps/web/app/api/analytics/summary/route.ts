...
export async function GET(request: Request) {
  ...
  const { upstreamResponse, sawBackend404 } = await fetchAnalyticsWithFailover({
    ...
    endpointPath: "/analytics/summary",
    suffix,
  });
    ...
    return NextResponse.json(
      {
        detail: sawBackend404
          ? "Analytics summary endpoint is updating. Please try again shortly."
          : "Unable to load analytics smart summary right now. Please try again shortly.",
      },
