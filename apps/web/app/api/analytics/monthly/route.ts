...
export async function GET(request: Request) {
  ...
  const { upstreamResponse, sawBackend404 } = await fetchAnalyticsWithFailover({
    ...
    endpointPath: "/analytics/monthly",
    suffix,
  });
    ...
    return NextResponse.json(
      {
        detail: sawBackend404
          ? "Monthly analytics endpoint is updating. Please try again shortly."
          : "Unable to load monthly analytics right now. Please try again shortly.",
      },
