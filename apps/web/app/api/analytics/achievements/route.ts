...
export async function GET(request: Request) {
  ...
  const { upstreamResponse, sawBackend404 } = await fetchAnalyticsWithFailover({
    ...
    endpointPath: "/analytics/achievements",
    suffix,
  });
    ...
    return NextResponse.json(
      {
        detail: sawBackend404
          ? "Achievement analytics endpoint is updating. Please try again shortly."
          : "Unable to load achievement analytics right now. Please try again shortly.",
      },
